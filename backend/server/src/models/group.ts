...
import {
    ...
    GetUsersWithGroupAccessQP,
    GetUsersWithGroupAccessR,
} from '../database/pg-typings/get-users-with-group-access';
import {
    changeGroupAccessQ,
    ChangeGroupAccessQP,
    ChangeGroupAccessR,
} from '../database/pg-typings/change-group-access';
...
export const GroupFns = {
    ...
    getFullGroupById: async (userId: number, groupId: number): Promise<FullGroup> => {
        ...
        return {
            ...
            childrenRepositories: resultRepositories.rows.map(
                ({ id, title, access, is_private, user_id, username }) => ({
                    id,
                    title,
                    access: bitMaskToRWA(access, is_private),
                    isPrivate: is_private,
    ...
    getUsersWithGroupAccess: async (userId: number, groupId: number): Promise<Array<User & { access: RWA }>> => {
        ...
        try {
            const client = await pg.connect();
            result = await client.query<GetUsersWithGroupAccessR, GetUsersWithGroupAccessQP>(getUsersWithGroupAccessQ, [
                groupId,
            ]);
            client.release();
        } catch (error) {
