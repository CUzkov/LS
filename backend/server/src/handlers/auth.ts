...
export const loginUser: ResponseCallback<ILoginUserProps> = async ({response, data, cookies}) => {
    ...
    const user = data?.email ? await UserFns.getUserByEmail(data.email) : await UserFns.getUserByUsername(data.username);
    ...
    getOkResponse(response, user);
};

export const checkAuth: ResponseCallback<{}> = async ({response}) => {
